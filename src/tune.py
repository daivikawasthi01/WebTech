...
Search space:
  ...
  hidden1      : categorical [32, 64, 128]
  hidden2      : categorical [16, 32, 64]
  hidden3      : categorical [0, 8, 16, 32]  (0 = skip 3rd layer)
...
def _objective(trial, csv_file: str, log_transform: bool):
    ...
    hp = {
        ...
        "hidden3":      trial.suggest_categorical("hidden3", [0, 8, 16, 32]),
    ...
    # Constraint: hidden2 must be <= hidden1; hidden3 (if used) <= hidden2
    if hp["hidden2"] > hp["hidden1"]:
        raise optuna.exceptions.TrialPruned()
    if hp["hidden3"] > 0 and hp["hidden3"] > hp["hidden2"]:
        raise optuna.exceptions.TrialPruned()
