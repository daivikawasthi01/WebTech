...
class FeatureSelectionGA:
    ...
    def _save_checkpoint(self, generation, population, best_chrom,
        ...
        ckpt = {
            ...
            # Cache keys are now (chromosome_tuple, split_seed_int).
            # Serialise as "bits|seed" e.g. "1,0,1,0|3"
            'cache':        {
                '|'.join(str(b) for b in k[0]) + '|seed|' + str(k[1]): list(v)
    ...
    def _load_checkpoint(self):
        ...
        try:
            ...
            for k_str, v in ckpt.get('cache', {}).items():
                try:
                    if '|seed|' in k_str:
                        bits_part, seed_part = k_str.split('|seed|')
                        chrom = tuple(int(b) for b in bits_part.split('|') if b)
                        seed  = int(seed_part)
                        restored_cache[(chrom, seed)] = tuple(v)
                    else:
                        # Legacy format (chromosome-only key) — skip, no seed info
                        pass
                except Exception:
                    pass
    ...
    # Fitness
    # -----------------------------------------------------------------------
    def calculate_fitness(self, chromosome: tuple,
                          split_seed: int = 42) -> tuple:
        """
        Returns (fitness, mse). Memoised per (chromosome, split_seed).

        split_seed is varied per generation by the evolve() loop so the GA
        cannot overfit to one fixed set of k-fold splits. Without this,
        k-fold with a constant seed rewards chromosomes that happen to look
        good on those specific validation folds, not ones that generalise.
        The cache key includes the seed so inter-generation comparisons remain
        valid (same generation → same seed → fair ranking within a generation).

        n_epochs=100: enough for the ANN to converge reliably.
        use_kfold=True: averages out split variance within each evaluation.
        """
        cache_key = (chromosome, split_seed)
        ...
        self.evaluation_cache[cache_key] = (fitness, mse)
        return fitness, mse
