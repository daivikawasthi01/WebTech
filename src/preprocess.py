...
def preprocess_dataset(input_csv: str, output_csv: str) -> pd.DataFrame:
    ...
    # Include non_numeric_features in the concat so the NaN-drop index is shared.
    # Previously non_numeric_features used .loc[full.index] AFTER full had been
    # reset_index(drop=True), which meant it was indexing with 0,1,2,… against
    # the original df index — silently returning the wrong rows when rows had
    # been dropped and the index was non-contiguous.
    parts_for_drop = [identifiers, numeric_features, target]
    if non_numeric_features is not None:
        parts_for_drop.append(non_numeric_features)

    full = pd.concat(parts_for_drop, axis=1)
    full = full.dropna(subset=[id_col, target_col] + numeric_features.columns.tolist())
    full = full.reset_index(drop=True)
    print(f"  After NaN drop         : {full.shape}")

    identifiers      = full[id_col]
    target           = full[target_col]
    numeric_features = full[numeric_features.columns.tolist()]

    if non_numeric_features is not None:
        non_numeric_features = full[non_numeric_features.columns.tolist()]
    ...
    # Column order: file_name | numeric features | target_bug_proneness | repo (if present)
    # Putting metadata (repo) AFTER the target keeps the numeric feature block
    # contiguous and avoids breaking any code that still uses iloc[:,1:-1].
    # ann_model.py uses select_dtypes so position is irrelevant there, but this
    # ordering is safer for anything else that reads the CSV positionally.
